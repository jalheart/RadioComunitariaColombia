...
| # | Tarea | Archivos afectados | Razón |
|---|---|---|---|
| 1 | Arreglar `streamUrl` + `_extractPort` | `domain/entities/radio_station.dart`, `infrastructure/datasources/radio_station_remote_datasource.dart` | `_extractPort` extrae `?p=8287` de una URL tipo web y `streamUrl` lo interpreta como puerto TCP — la URL generada es incorrecta y el stream no se reproduce |
